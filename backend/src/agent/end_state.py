...
Rules:
- If composite >= 13: end_state MUST be "systemic_collapse"
...
async def assess_end_state(
    ...
    llm = get_llm(temperature=0.3, timeout=180)  # longer timeout for large response
...
def _fallback_end_state(
    ...
    if score >= 13:
        briefing = (
            "The global crisis monitor indicates a state of systemic collapse (composite score {score}/16). "
        ...
        briefing = (
            "The crisis monitor shows elevated stress (composite score {score}/16) with fragmented stability across regions. "
        ...
        briefing = (
            "The global risk picture shows moderate stress (composite score {score}/16). "
        ...
        briefing = (
            "The global crisis monitor shows a contained situation (composite score {score}/16). "
    ...
    return {
        ...
        "q1": {
            "question": "Is this a controlled correction or the start of a structural break?",
            "answer": f"With composite score {score}/16, this represents a {'contained situation' if score < 5 else 'potential structural shift'}. Monitor key indicators for confirmation.",
            "verdict": "controlled_correction" if score < 5 else ("transitional" if score < 9 else "structural_break"),
        },
        "q2": {
            "question": "Which region or asset class is the proximate trigger if a cascade begins?",
            "answer": "Analysis unavailable — using composite heuristic.",
            "trigger_region": "Multiple regions" if score >= 9 else "None identified",
            "trigger_probability": min(score / 16, 0.9),
        },
        "q3": {
            "question": "What is the probability of a coordinated global recession within 6 months?",
            "answer": f"Based on composite score {score}/16, recession probability estimated at {recession_prob:.0%}.",
            "probability": recession_prob,
        },
        "q4": {
            "question": "Are policymakers responding adequately or are they behind the curve?",
            "answer": "Analysis unavailable — using composite heuristic.",
            "assessment": "adequate" if score < 5 else ("behind_curve" if score < 13 else "ineffective"),
        },
        "q5": {
            "question": "What single indicator would you watch most closely over the next 7 days?",
            ...
            "indicator": "VIX" if score < 9 else "IG OAS Credit Spread",
...
if __name__ == "__main__":
    fb = _fallback_end_state(
        {"composite": 2, "interpretation": "monitor"},
